...
function updatewindow() {
  ...
  chrome.storage.sync.get('charstore', function(items){
    ...
    document.getElementById("total-words").innerHTML = "Total words typed: " + parseInt(items.charstore[' '],10);
    document.getElementById("total-chars").innerHTML = "Total Characters Typed: " + parseInt(total);
  ...
  chrome.storage.sync.get('avg', function(items){
    ...
    if (items.avg && items.avg.length > 0){
      ...
      for (var i = 0; i< items.avg.length; i++){
        ...
        avgcpm += items.avg[i].val.cpm;
      }
      avg /= items.avg.length;
      avgcpm /= items.avg.length
      computePerc(avg);
      document.getElementById("average-wpm").innerHTML =  "Average WPM: " + parseInt(avg,10);
      document.getElementById("average-cpm").innerHTML =  "Average CPM: " + parseInt(avgcpm,10);
    }
  });
      
  }
function setPerc(value){
	document.getElementById("speed-percentile").innerHTML = "Typing Speed Percentile: " + parseInt(value,10) + "%";
}
